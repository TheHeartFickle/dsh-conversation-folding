window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function AssistantNodeView(props) {
			...
			var nodes = chat ? chat.nodes : undefined;
		...
		// 本插件注册进 conversation.chat.node 的 entry 组件（apply 时赋值）。
		// 0.1.2 起 priority 由宿主按注册顺序自动分配，查找官方 renderer 时
		// 只能按组件身份排除自身。
		var ownEntries = { context: null, toolCall: null };

		function findShippedToolCallTree(slots) {
			var entries = slots.entries("conversation.chat.node");
			for (var i = 0; i < entries.length; i++) {
				var e = entries[i];
				if (e.options.key === "tool-call" && e.component !== ownEntries.toolCall) return e.component;
			}
			return undefined;
		}
			...
			return function (key, owner, opts) {
				...
				for (var i = 0; i < entries.length; i++) {
					if (entries[i].options.key === opts.entryKey) {
						// 全量透传组合 props（含 useChat/useSessions/sessionId 等 standard props），
						// toolview 组件按需取用；owner 在后覆盖同名字段
						return React.createElement(entries[i].component, Object.assign({}, props, owner, { t: props.t }));
					}
				}
		...
		function renderShippedToolCall(slots, node, props) {
			...
			return React.createElement(ShippedToolCallTree, Object.assign({}, props, {
		...
		// ---------- 上下文注入等辅助节点折叠 ----------
		function findShippedNodeTree(slots, key, ownComponent) {
			var entries = slots.entries("conversation.chat.node");
			for (var i = 0; i < entries.length; i++) {
				var e = entries[i];
				if (e.options.key === key && e.component !== ownComponent) return e.component;
			}
			return undefined;
		}

		function renderShippedNode(slots, key, node, props, ownComponent) {
			var ShippedNodeTree = findShippedNodeTree(slots, key, ownComponent);
		...
		function ContextNodeView(props) {
			...
			return renderShippedNode(props.slots, props.node.kind, props.node, props, ownEntries.context);
		}
		...
		function apply(ctx) {
			...
			// DSH 0.1.2 移除了 connection.hostDescription：官方 tool-call 树改用
			// useHostInfo（hooks.hostInfo）读取宿主 home，供 ~ 路径展开等场景使用。
			var hostInfo = {
				getSnapshot: function () { return remote.$host; },
				subscribe: function (listener) { return ctx.on("connection/reset", listener); }
			};
			var toolInject = function () {
				return { hooks: { hostInfo: hostInfo } };
			};
			...
			// 0.1.2 起 slots.register 的 priority 由宿主按注册顺序自动分配
			//（显式值会被覆盖，后注册者优先）；插件 bundle 晚于官方包加载，
			// 天然 shadow 官方 renderer。
			var contextEntry = function (props) {
				return React.createElement(ContextNodeView, Object.assign({}, props, { slots: slots }));
			};
			ownEntries.context = contextEntry;
			slots.inject("conversation.chat.node", function () {
				return slots.register(
					{ name: "conversation.chat.node", key: "context", locale: "conversation" },
					contextEntry
				);
			});
			...
			var toolCallEntry = function (props) {
				return React.createElement(ToolCallGroup, Object.assign({}, props, { slots: slots }));
			};
			ownEntries.toolCall = toolCallEntry;
			slots.inject("conversation.chat.node", function () {
				return slots.register(
					{ name: "conversation.chat.node", key: "tool-call", locale: "conversation", inject: toolInject },
					toolCallEntry
				);
			});
		}
