window.__ModuleLoader__.load({
	id: "@the-heart-fickle/dsh-conversation-folding-plugin",
	factory: (require) => {
		var module = { exports: {} };
		...
		function hasTextOfTurn(order, nodes, turn) {
			for (var i = 0; i < order.length; i++) {
				var node = nodes.get(order[i]);
				if (!node || node.kind !== "assistant-step" || turnOf(node) !== turn) continue;
				if (visibleBlocksOf(node.data && node.data.blocks).length > 0) return true;
			}
			return false;
		}
		...
		function AssistantNodeView(props) {
			...
			if (mode === "all") {
				...
				var hasFinal = hasTextOfTurn(order, nodes, turn);
				var parts = [];
				if (firstProcess) {
				...
				if (expandedTurn) {
					...
					// 正文节点独立显示，但隐藏其中的 reasoning/tool-call 过程。
					parts.push(React.createElement(AssistantMarkdown, {
						blocks: data.blocks,
						hideReasoning: true,
						streaming: data.status === "running",
		...
		function ToolCallGroup(props) {
			...
			if (mode === "all") {
				...
				if (!hasTextOfTurn(order, nodes, turn) && isLatestProcessNode(order, nodes, node)) {
