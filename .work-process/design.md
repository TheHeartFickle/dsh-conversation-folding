...
interface SegmentIndex {
  // 不提供单一 segmentInfoByNode：混合 [reasoning + text] 节点同时属于上一段（过程）和下一段（边界），
...
- `processSegmentOf(nodeKey)`：节点作为“过程”所属的段；仅 process 或混合 `[reasoning + text]` body 有值。
- `boundarySegmentOf(nodeKey)`：节点作为“边界”开启的下一段；仅边界节点（user/steering/body）有值。
- 混合 body 同时有两个角色，所以**不使用单一 `segmentIdByKey`**。
...
function clearSessionSegments(sessionId): void;
function observeSessionId(sessionId): void; // 首次切换时清空 activeSessionId 整块
```

清理策略（不依赖未确认的 DSH session 卸载钩子）：

- 保存 `activeSessionId`；
- 当 `props.sessionId` 首次变成新值时，调用 `clearSessionSegments(activeSessionId)`，再更新 `activeSessionId`；
- 当前 session 只保留自己的条目。
...
- 纯正文 body 不是 process。
- 混合 `[reasoning + text]` 是 process，也同时是 body 边界。
- `segments.js` 需要自包含以下纯函数：`hasReasoning`、`toolCallName`、`isSkillToolCall`、`isProcessNode`、`hasVisibleBody`、`segmentIdOfBoundary`、`isFrontEdgeSegment`、`buildSegmentIndex` 及渲染决策辅助函数；`client.js` 中相应旧定义 `hasReasoning` / `isSkillToolCall` / `isProcessNode` 若与生成块重复，必须删除并统一使用生成块。
...
| `shouldHideReasoning(segmentInfo, node, expanded)` | 前缘恒 false；普通 collapsed 且有 reasoning 时 true；否则 false |
...
- `mode === "all"`：
  ...
  - 混合 body：
    - collapsed：`hideReasoning: true`，只渲染正文；
    - expanded：完整渲染；
    - 若所属过程段 `isFrontEdge === true`，`hideReasoning` 必须为 `false`。
  - 若 `index.processSegmentOf(node.key)` 对应的 segment 满足 `shouldRenderFoldBar`，先渲染 `ProcessFold`（父组件会因 index/segmentVersion 变化重渲染）。
  - 纯 process 节点：
    - 若 `shouldHideProcessContent(...)` 为 true，返回非空 `HiddenProcessNode()`（内部 `data-dsh-hidden-process`，不隐藏外层 seat）；
    - 展开时渲染完整内容；
    - 若当前段无正文且 collapsed，仅最后 process 可见作为预览。
...
- `mode === "toolcall"` / `"none"`：保持现有分支，不接入 segment 状态。

### 6.2 `ToolCallGroup`

- `mode === "all"`：
  - 使用与 `AssistantNodeView` 相同的 `buildSegmentIndex` 订阅（`turnKeys` / `nodeValues` 换引用触发重算）；
  - `skill` tool-call：`segmentExpanded(sessionId, segmentId) || includesAux("skill")` 时可见，否则返回非空 `HiddenAuxNode()`；
  - 非 skill tool-call：
...
- `mode === "all"`：
  - 使用与 `AssistantNodeView` 相同的 `buildSegmentIndex` 订阅（`turnKeys` / `nodeValues` 换引用触发重算）；
  - segment 归属：**context 节点本身不是过程也不是正文边界**，取“它落在哪个段”用 `index.processSegmentOf(node.key)` 或按节点在 `turnKeys` 中的位置查询所在 `segmentInfo`；
  - 可见条件：`segmentExpanded(sessionId, segmentId) || includesAux("context")`；
  - 判断为不可见时返回**非空占位** `HiddenAuxNode()`（内部 `data-dsh-hidden-aux`），不返回 null，避免 DSH `.flowItem:empty` 隐藏外层锚点；
...
- 改为：
  ...
  ```
- 保留 DSH 外层 `data-chat-anchor-key` / `ChatNodeSeat` 始终挂载。
- **所有隐藏占位必须是非空元素**：
  - `HiddenProcessNode()` 返回 `<div data-dsh-hidden-process></div>`；
  - `HiddenAuxNode()` 返回 `<div data-dsh-hidden-aux></div>`；
  - 不得返回 `null` / 空片段，否则 DSH 的 `.flowItem:empty { display:none }` 仍会隐藏外层锚点。
- 被折叠的过程行：外层 seat 仍挂载，内部过程内容用 `[data-dsh-hidden-process]` 局部隐藏。
- 若实测 `loadOlderAnchored` 仍依赖高度变化，再仅为隐藏行加最小占位高度；绝不隐藏外层锚点元素。
- 折叠栏右侧步骤数样式：
  ```css
  .dsh-turnfold-count { color: var(--dsw-alias-label-tertiary); font-size: 0.75rem; }
  ```
- **`toolcall` / `none` 模式兼容**：
  - `all` 模式统一走 `data-dsh-hidden-process` / `data-dsh-hidden-aux`；
  - `toolcall` 模式原先用 `data-dsh-hidden-toolcall` 的隐藏规则也改为非空占位 `data-dsh-hidden-process`，以保持“外层 seat 不隐藏、锚点保留”的一致契约；
...
- 混合 reasoning+text 节点：`processSegmentOf` 指向上一段、`boundarySegmentOf` 指向下一段，且可作为首过程渲染折叠栏；
